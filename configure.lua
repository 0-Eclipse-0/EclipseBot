...
print "--------------------"
print "Welcome to the EBot configuration script."
...
if db_exists then
    print "It looks like there is already a database for EBot set up. Don't worry, this script won't ruin it."
end
...
if config_exists then
    print "It looks like EBot is already configured. Do you want to skip the configuration step?"
    local answer
    repeat
        io.write ">> "
        io.flush()
...
if not skip_config then
    ...
    repeat
        ...
        repeat
            print "What channel should the bot watch? This should be the username of the channel (i.e. EBot for the EBot channel)"
            io.write ">> "
            io.flush()
            channel = io.read()
    ...
    if not config_dist then
        print "Uh oh! I couldn't read config/config.lua.dist. I suggest trying to download it from https://github.com/redxdev/EBot/blob/master/config/config.lua.dist and running this script again."
        return
    end
...
if not skip_admin then
    print "Now it is time to setup an administrator. This user will be able to access all of EBot's functions."
...
print "All done! EBot should be ready to go. You can launch EBot by running \"lua EBot.lua\" or you can run this configuration again."
